...
# set sample size - if necessary, sample a subset of safety calls to actually compute clustering
num.samples = 5000
...
# vector to track whether city has selected data
cities.active = rep(0, length(cities))
...
cities.active.use = which(cities.active == 1)

plots.ls = list()
ginis = rep(0, length(cities.active.use))

for(c in 1:length(cities.active.use)) {
  location.cluster.use = location.clusters.ls[[cities.active.use[c]]]
  
  if(length(location.cluster.use) > 0) {
    ...
    g = round(gini(cluster.counts.df$Count),2)
    ginis[c] = g
    ...
    plots.ls[[c]] = ggplot2::ggplot(plot.df, ggplot2::aes(x = Safety, y = Safety.Cluster)) + ggplot2::geom_bar(stat = "identity", fill = "light blue", color = "black") +
      ggplot2::labs(x = "Number of Safety Calls", y = "Number of Clusters", title = paste0(cities[cities.active.use[c]]," ",safety.use," calls, Gini = ",g)) +
...
gridExtra::grid.arrange(grobs = plots.ls)


# COMMAND ----------

# get monthly Ginis across all calls for a given city
...
# add year column for easy yearly aggregates
safety.city.df = safety.city.df %>%
  mutate(Year.Month = as.Date(format(dateTime, "%Y-%m-01")))

head(safety.city.df)

# COMMAND ----------

safety.city.df$Longitude = as.numeric(safety.city.df$longitude)
safety.city.df$Latitude = as.numeric(safety.city.df$latitude)

unique.months = unique(safety.city.df$Year.Month)
m = 1
safety.city.month.df = safety.city.df[safety.city.df$Year.Month == unique.months[m],]

rows.to.remove = which(is.na(safety.city.month.df$Longitude))

  if(length(rows.to.remove) > 0) {
    safety.city.month.df = safety.city.month.df[-rows.to.remove,]  
  }

lat.long.mat = as.matrix(safety.city.month.df[,c("Longitude","Latitude")])
num.samples = 5000
if(nrow(lat.long.mat) > 5000) {
      sample.rows = base::sample(1:nrow(lat.long.mat),num.samples)
      lat.long.mat = lat.long.mat[sample.rows,]  
    }


safety.city.dist = distm(lat.long.mat, fun=distHaversine)
...
monthly.ginis = rep(0,length(unique.months))

for(m in 1:length(unique.months)) {
  ...
  num.samples = 5000
  if(nrow(safety.city.month.df) > 5000) {
      sample.rows = base::sample(1:nrow(safety.city.month.df),num.samples)
      safety.city.month.df = safety.city.month.df[sample.rows,]  
  }
  
  lat.long.mat = as.matrix(safety.city.month.df[,c("Longitude","Latitude")])
  safety.city.dist = distm(lat.long.mat, fun=distHaversine)

  safety.city.clust <- hclust(as.dist(safety.city.dist), method="complete")

  # define the distance threshold
  distance.threshold=1000

  # define clusters based on distance threshold
  safety.city.clusters <- cutree(safety.city.clust, h=distance.threshold)

  location.clusters = data.frame(Cluster = safety.city.clusters, Latitude = safety.city.month.df$latitude,Longitude = safety.city.month.df$longitude)
  
  # get the number of clusters with each number of crimes
  cluster.counts.df = data.frame(Cluster = names(table(location.clusters$Cluster)), Count = as.numeric(table(location.clusters$Cluster)))
  ...
  cluster.df = data.frame(Safety.Cluster = num.clusters, Safety = 1:length(num.clusters))

  monthly.ginis[m] = round(gini(cluster.df$Safety.Cluster),2)
...
# get monthly counts of each call type
safetyDFCity = as.DataFrame(safety.city.df)

safetyDFCity.Monthly = groupBy(safetyDFCity, c("Year.Month","category")) %>%
  summarize(safetyDFCity, Count = n())
...
call.month = rep(0, length(unique.months))
...
for(m in 1:length(unique.months)) {
  ...
  call.month[m] = nrow(safety.city.month.df)
}  
...
call.gini.df = data.frame(Calls = call.month, Gini = monthly.ginis, Month = unique.months)
call.gini.df = call.gini.df[call.gini.df$Month > "2011-01-01",]
call.gini.df = call.gini.df[call.gini.df$Month < "2018-06-01",]

call.gini.df$Calls = scale(call.gini.df$Calls)
call.gini.df$Gini = scale(call.gini.df$Gini)

#cor(call.gini.df$Gini, call.gini.df$Calls)

plot.df = melt(call.gini.df, id.vars = "Month")

#ggplot(plot.df, aes(x = Month, y = value, color = variable)) + geom_line()
ggplot(call.gini.df, aes(x = Calls, y = Gini)) + geom_point()
